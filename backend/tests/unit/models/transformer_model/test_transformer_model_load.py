...
def test_load_success(model_info):
    # Directly use the 'dir' from model_info to ensure consistency
    expected_model_dir = model_info['dir']
    ...
    with patch('backend.models.transformer_model.os.path.exists', return_value=True) as mock_exists, \
        ...
        mock_construct_pipeline.assert_called_once_with(model_info.get('pipeline_tag'))
        ...
        assert transformer_model.config == model_info.get('config', {})
