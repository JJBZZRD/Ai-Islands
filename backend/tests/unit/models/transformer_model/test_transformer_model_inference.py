...
@pytest.fixture
def transformer_model(model_info):
    model = TransformerModel(model_id="Qwen/Qwen2-0.5B-Instruct")
    model.config = copy.deepcopy(model_info['config'])  # Use deep copy here
    model.pipeline = MagicMock()
    model.pipeline.task = model_info.get('pipeline_tag')  # Set the task attribute
